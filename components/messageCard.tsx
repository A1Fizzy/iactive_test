...
interface MessageCardProps {
  ...
  onMove: (messageId: number | string, targetColumn: ColumnType) => void;
}
...
const MessageCard: React.FC<MessageCardProps> = ({
  ...
  const handleToggleFavorite = () => {
    dispatch(toggleFavorite(message.id));
  };
  ...
  const handleMove = (targetColumn: ColumnType) => {
    if (targetColumn !== currentColumn) {
      setIsLeaving(true);
      setTimeout(() => {
        onMove(message.id, targetColumn);
        setIsLeaving(false);
      }, 300);
    }
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-row relative gap-3 justify-stretch w-full">
          <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#808080] flex items-center justify-center text-white font-medium">
            {avatarLetter}
          </div>
          <div>
            <div className="font-normal text-[#808080] text-[16px]">
              {message.author}
            </div>
            <div className="flex flex-row absolute right-1 top-1 gap-1 flex-shrink-0">
              <button
                onClick={() => setIsMinimized(!isMinimized)}
                className="text-xs text-[#808080] transition-all duration-300 ease-in-out opacity-40 hover:text-blue-300"
              ...
              <button
                ...
                className="text-[#808080] transition-all duration-300 ease-in-out opacity-40 hover:text-blue-300 text-lg"
                title="Настройки"
              >
                <svg
                  width="24"
              ...
              <button
                ...
                className={`text-xl transition-all duration-300 ease-in-out ${
                  message.isFavorite
                    ? "text-blue-500"
                    : "text-[#808080] opacity-40 hover:text-blue-300"
                }`}
                title="Избранное"
              >
                <svg
        ...
        {(["left", "center", "right"] as const).map((col) => (
          <button
            ...
            className={`px-2 py-1 text-xs rounded-xs border ${
              currentColumn === col
                ? "bg-[#0088EE] text-white border-[#0088EE] hover:bg-blue-300"
                : "border-[#0088EE] text-[#0088EE] hover:bg-blue-300 hover:text-white"
            }`}
          >
            {col === "left" ? "Левый" : col === "center" ? "Центр" : "Правый"}
          </button>
        ))}
      </div>
      ...
      {isMenuOpen && (
        <div
          ref={menuRef}
          className="absolute right-3 top-8 z-10 bg-white border border-gray-200 rounded shadow-lg w-40"
        >
          <div
            onClick={handleCopyText}
            className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm text-[#808080]"
          >
            Скопировать текст
          </div>
          <div
            onClick={handleDelete}
            className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm text-red-600"
          >
            Удалить
          </div>
        </div>
